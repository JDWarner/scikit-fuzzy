...
principles to generate complex behavior from an intuitive set of expert rules.
...
tip_hi = fuzz.trimf(x_tip, [0, 0, 12.5])
...
Now, to make these triangles useful, we define the fuzzy relationship between
the input and output variables. For the purposes of our example, we consider
three simple rules:

1. If the food is bad or the service is poor, the tip will be low
2. If the service is merely acceptable, the tip will be medium
3. If the food is great or the service is amazing, the tip will be high.
...
What would the tip be if the food quality was 6.5, while service was 9.8?
...
maximum operator. This step is also known as "rule aggregation."
...
ax0.plot([tip, tip], [0, tip_activation], 'k', linewidth=1.5, alpha=0.1)
